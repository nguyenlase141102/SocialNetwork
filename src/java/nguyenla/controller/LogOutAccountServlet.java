...
public class LogOutAccountServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (session != null) {
                session.removeAttribute("listManage");
                
                session.removeAttribute("NumberComment");
                session.removeAttribute("listAllInter");
                session.removeAttribute("listAllArticle");
//                session.invalidate();
                url = LOGINPAGE;
//                interDAO.deleteNull();
            }
            ...
            System.err.println("Error messages (Logout Servlet: ) " + e.getMessage());
        }
    }
