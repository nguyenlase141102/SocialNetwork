...
public class DeletePostServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String idArticle = request.getParameter("hiddenIdArticle").trim();
            int idArticleInt = Integer.parseInt(idArticle);
            ArticleDAO artiDAO = new ArticleDAO();

            if (interAccountDAO.deleteArticle(idArticleInt)) {
                if (artiDAO.deletePost(idArticleInt)) {
                    url = LOADHOMEPAGE;
                }
            ...
            System.err.println("Error messages (Delete post): " + e.getMessage());
        }
    }
